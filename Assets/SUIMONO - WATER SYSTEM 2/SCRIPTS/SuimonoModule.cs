...
*/
		...
		void LateUpdate (){
			...
			if (fxRippleObject != null){
				if (IsValidUnityLayer(layerScreenFXNum)) fxRippleObject.layer = layerScreenFXNum;
			}


			//SET COMPONENT LAYERS
			if (suimonoModuleLibrary.normalsCamObject != null) suimonoModuleLibrary.normalsCamObject.cullingMask = LayerMaskFromLayer(layerScreenFXNum);
			if (suimonoModuleLibrary.wakeCamObject != null) suimonoModuleLibrary.wakeCamObject.cullingMask = LayerMaskFromLayer(layerScreenFXNum);
			...
			if (suimonoModuleLibrary.transCamObject != null){
				transLayer = RemoveLayerFromMask(transLayer, layerWaterNum); //remove water layer from transparent mask
				transLayer = RemoveLayerFromMask(transLayer, layerDepthNum); //remove Depth layer from transparent mask
				transLayer = RemoveLayerFromMask(transLayer, layerScreenFXNum); //remove Screen layer from transparent mask
			...
			causticLayer = RemoveLayerFromMask(causticLayer, layerWaterNum);
			causticLayer = RemoveLayerFromMask(causticLayer, layerDepthNum);
			causticLayer = RemoveLayerFromMask(causticLayer, layerScreenFXNum);
		...
		private static bool IsValidUnityLayer(int layerIndex){
			return layerIndex >= 0 && layerIndex <= 31;
		}


		private static int LayerMaskFromLayer(int layerIndex){
			return IsValidUnityLayer(layerIndex) ? (1 << layerIndex) : 0;
		}


		private static int RemoveLayerFromMask(int mask, int layerIndex){
			return IsValidUnityLayer(layerIndex) ? (mask & ~(1 << layerIndex)) : mask;
		}
